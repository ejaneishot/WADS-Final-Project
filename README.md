...
- **Features:** Quiz system — `/quiz`, `/quiz/[slug]` per-track quiz pages, `src/lib/quiz-data.ts` (quiz content file covering all 6 career tracks); career track milestone component (`src/components/careers/CareerMilestones.tsx`) synced with quiz track data; subscription pricing page (`/pricing`); subscription plan management UI (free and pro switching); subscription plan synced with quiz availability access, final project report
