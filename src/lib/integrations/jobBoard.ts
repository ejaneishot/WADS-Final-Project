...
export const publicJobListingSchema = z.object({
  id: z.string(),
  title: z.string(),
  company: z.string(),
  url: z.string().url(),
...
export type PublicJobListing = z.infer<typeof publicJobListingSchema>;
...
function buildRemotiveUrl(search: string): string {
...
async function fetchFromRemotive(
  ...
  const url = buildRemotiveUrl(search);
...
/**
 * Server-only job listings for a career track (never calls third-party APIs from the browser).
 */
